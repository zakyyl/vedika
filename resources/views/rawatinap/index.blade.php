...
<form method="GET" action="{{ route('rawatinap.index') }}" class="row g-3 mb-4">
  <div class="col-md-4">
  ...
  <div class="col-md-3">
    <input type="date" name="tgl_sampai" class="form-control" value="{{ request('tgl_ampai') }}">
  </div>
  <div class="col-md-2">
...
<div class="col-12">
  <div class="card overflow-hidden">
      ...
      <table class="table table-sm">
        ...
        <tbody>
          ...
          <tr>
            ...
            {{-- <td class="text-truncate">{{ $data->umur }}</td> --}}
            <td class="text-truncate">{{ $data->nm_poli }}</td>
            {{-- <td class="text-truncate">{{ $data->png_jawab }}</td> --}}
            <td><a href="{{ route('rawatinap.detail', urlencode($data->no_rawat)) }}" class="btn btn-primary w-100">Detail Data</a></td>
          </tr>
          @empty
          <tr>
    ...
    <div class="d-flex justify-content-center my-3">
      {{ $rawatInap->links('pagination::bootstrap-5') }}
    </div>
  </div>
</div>
@endsection
