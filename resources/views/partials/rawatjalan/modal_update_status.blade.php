<!-- partials/rawatjalan/modal_update_status.blade.php -->
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('rawatjalan.update_status', $data->no_rawat) }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label>No. SEP</label>
                        <input type="text" class="form-control" value="{{ $vedikaData->nosep ?? 'Belum ada data SEP' }}" readonly>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        @if(isset($vedikaData) && $vedikaData->status)
                            <small class="text-muted">Status saat ini: <strong>{{ $vedikaData->status }}</strong></small>
                        @endif
                    </div>

                    <div class="form-group">
                        <label>Catatan</label>
                        <textarea name="catatan" class="form-control" rows="4" placeholder="Masukkan catatan...">{{ $vedikaData->catatan ?? '' }}</textarea>
                    </div>
                    ...
                    @if(isset($vedikaData) && ($vedikaData->status || $vedikaData->catatan))
                        <div class="alert alert-info">
                            ...
                            @if($vedikaData->status)
                                <p><strong>Status:</strong> <span class="badge badge-info">{{ $vedikaData->status }}</span></p>
                            @endif
